...
void SeplosBms::on_telemetry_data_(const std::vector<uint8_t> &data) {
  ...
  float total_voltage = (float) seplos_get_16bit(9) * 0.01f;
  ...
  this->publish_state_(this->battery_capacity_sensor_, (float) seplos_get_16bit(offset + 6) * 0.01f);
  this->publish_state_(this->state_of_charge_sensor_, (float) seplos_get_16bit(offset + 10) * 0.1f);
  this->publish_state_(this->rated_capacity_sensor_, (float) seplos_get_16bit(offset + 8) * 0.01f);
